...
(function () {
  ...
  function makeSprite(kind) {
    ...
    if (kind === 0) {                                   /* golden dust */
      ...
      gd.addColorStop(0, 'rgba(248,238,210,1)');
      gd.addColorStop(0.22, 'rgba(212,184,106,0.72)');
      gd.addColorStop(0.58, 'rgba(184,148,42,0.20)');
      gd.addColorStop(1, 'rgba(184,148,42,0)');
      g.fillStyle = gd;
      g.beginPath(); g.arc(h, h, h, 0, TAU); g.fill();

    } else if (kind === 1) {                            /* glowing star — warm white */
      var gs = g.createRadialGradient(h, h, 0, h, h, h * 0.34);
      gs.addColorStop(0, 'rgba(252,248,236,1)');
      gs.addColorStop(0.5, 'rgba(212,184,106,0.55)');
      gs.addColorStop(1, 'rgba(184,148,42,0)');
      g.fillStyle = gs;
      g.beginPath(); g.arc(h, h, h * 0.34, 0, TAU); g.fill();

      var gsp = g.createRadialGradient(h, h, 0, h, h, h);
      gsp.addColorStop(0, 'rgba(248,244,228,0.95)');
      gsp.addColorStop(0.35, 'rgba(212,184,106,0.34)');
      gsp.addColorStop(1, 'rgba(184,148,42,0)');
      g.fillStyle = gsp;
    ...
    } else {                                            /* soft sage bubble */
      var gb = g.createRadialGradient(h, h * 0.8, 0, h, h, h * 0.72);
      gb.addColorStop(0, 'rgba(180,210,180,0.28)');
      gb.addColorStop(0.72, 'rgba(90,122,90,0.07)');
      gb.addColorStop(1, 'rgba(90,122,90,0)');
      g.fillStyle = gb;
      g.beginPath(); g.arc(h, h, h * 0.72, 0, TAU); g.fill();

      g.strokeStyle = 'rgba(150,190,150,0.38)';
      g.lineWidth = 1.6;
